...
​     ArrayList(Collection<? extends [E](mk:@MSITStore:G:%5C????????????%5Cjdk6.ZH_cn.chm::/j2se6/api/java/util/ArrayList.html)> c)：构造一个包含指定 collection 的元素的列表，这些元素是按照该 collection 的迭代器返回它们的顺序排列的。
